...
outFolder <- "output/cutting_investigation"
# dir.create(path = outFolder)
aggFolder <- "output/cutting_investigation/aggFolder"
# dir.create(path = aggFolder)

# Clears the previous run CSVs
unlink("output/cutting_investigation/*")
unlink("output/cutting_investigation/aggFolder/*")
...
cutting_rate <- list(0,0.2,0.4,0.6,0.8,1)
run <- list(1,2,3,4,5,6)
newphi <- c("HHBB" = 0, "WHBB" = 0)

for (i in 1:length(cutting_rate)) {
